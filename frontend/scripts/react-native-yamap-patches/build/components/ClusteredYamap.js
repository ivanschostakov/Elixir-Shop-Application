...
var ClusteredYamap = /** @class */ (function (_super) {
    ...
    ClusteredYamap.prototype.resolveMarkerIcons = function (markerIcons) {
        var _this = this;
        if (!markerIcons) {
            return undefined;
        }
        var resolvedEntries = Object.entries(markerIcons)
            .flatMap(function (_a) {
            var iconKey = _a[0], imageSource = _a[1];
            var uri = _this.resolveImageUri(imageSource);
            return uri ? [[iconKey, uri]] : [];
        });
        return resolvedEntries.length > 0 ? Object.fromEntries(resolvedEntries) : undefined;
    };
    ClusteredYamap.prototype.getProps = function () {
        var _a = this.props, clusteredMarkers = _a.clusteredMarkers, markerIcons = _a.markerIcons, userLocationIcon = _a.userLocationIcon, restProps = __rest(_a, ["clusteredMarkers", "markerIcons", "userLocationIcon"]);
        var props = __assign(__assign({}, restProps), { clusteredMarkers: (clusteredMarkers !== null && clusteredMarkers !== void 0 ? clusteredMarkers : []).map(function (_a) {
                var point = _a.point, data = _a.data, iconKey = _a.iconKey;
                return ({
                    point: point,
                    data: data,
                    iconKey: iconKey,
                });
            }), clusteredMarkerIcons: this.resolveMarkerIcons(markerIcons), onRouteFound: this.processRoute, onCameraPositionReceived: this.processCameraPosition, onVisibleRegionReceived: this.processVisibleRegion, onWorldToScreenPointsReceived: this.processWorldToScreenPointsReceived, onScreenToWorldPointsReceived: this.processScreenToWorldPointsReceived, userLocationIcon: userLocationIcon ? this.resolveImageUri(userLocationIcon) : undefined });
