...
class ClusteredYamapViewManager internal constructor() : ViewGroupManager<ClusteredYamapView>() {
    ...
    override fun getExportedCustomBubblingEventTypeConstants(): MutableMap<String, Any>? {
        return MapBuilder.builder<String, Any>()
            ...
            .put(
                "onMarkerPress",
                MapBuilder.of("phasedRegistrationNames", MapBuilder.of("bubbled", "onMarkerPress"))
            )
            .put(
                "screenToWorldPoints",
                MapBuilder.of(
    ...
    fun setClusteredMarkers(view: View, points: ReadableArray?) {
        if (points == null) {
            castToYaMapView(view).setClusteredMarkers(null)
            return
        }
    ...
    @ReactProp(name = "clusteredMarkerIcons")
    fun setClusteredMarkerIcons(view: View, icons: ReadableMap?) {
        if (icons == null) {
            castToYaMapView(view).clearClusteredMarkerIcons()
            return
        }

        val resolvedIcons = HashMap<String, String>()
        val iterator = icons.keySetIterator()
        while (iterator.hasNextKey()) {
            val iconKey = iterator.nextKey()
            icons.getString(iconKey)?.let { resolvedIcons[iconKey] = it }
        }

        castToYaMapView(view).setClusteredMarkerIcons(resolvedIcons)
    }
    ...
    @ReactProp(name = "markerSize", defaultFloat = 34f)
    fun setMarkerSize(view: View, markerSize: Float) {
        castToYaMapView(view).setMarkerSize(markerSize)
    }
