...
const MAX_NATIVE_PICKUP_MARKERS = 80000
const DOOR_DELIVERY_LOOKUP_DELAY_MS = 420
const DELIVERY_CLUSTER_MARKER_ICONS = {
    cdek: require("@/assets/icons/delivery-services/cdek.png"),
    yandex: require("@/assets/icons/delivery-services/yandex.png"),
} as const

function getClusteredMarkerPayload(rawData: unknown) {
    const code =
...
function getDoorDeliveryPoint(doorDeliveryDraft: DeliveryDoorDraft | null): Point | null {
...
function getPickupPoint(pickupPointDraft: DeliveryPickupDraft | null): Point | null {
    if (!pickupPointDraft) {
        return null
    }
...
function arePointsClose(left: Point | null, right: Point | null, precision = 0.00005) {
    if (!left || !right) {
        return false
    }
...
function areCameraPositionsEquivalent(
...
function getNearestPickupMarkers(markers: DeliveryMapMarker[], center: Point, limit: number) {
    ...
    return [...markers]
        .sort((left, right) => {
            return (
                getPointDistanceScore(left.point, center)
                - getPointDistanceScore(right.point, center)
            )
        })
        .slice(0, limit)
}
...
export default function DeliveryScreen() {
    ...
    }, [activeCountryCode, clearResults, moveToRegion, pickupPointDraft, router])
