import { StyleSheet } from "react-native"

import type { ThemePalette } from "@/theme/colors"
import { spacing } from "@/theme/spacing"

const PROFILE_PLACEHOLDER_BACKGROUND = "#EAF3FF"
const PROFILE_PLACEHOLDER_TEXT = "#0A84FF"

export const createProfileScreenStyles = (colors: ThemePalette) => StyleSheet.create({
    screen: {
        flex: 1,
        backgroundColor: colors.surface,
    },
    container: {
        flex: 1,
        backgroundColor: colors.surface,
    },
    content: {
        backgroundColor: colors.pageBackground,
        paddingHorizontal: 0,
        paddingTop: 0,
        paddingBottom: 0,
        gap: spacing.md,
    },
    heroCard: {
        position: "relative",
        overflow: "hidden",
        borderRadius: spacing.lg,
        borderTopLeftRadius: 0,
        borderTopRightRadius: 0,
        padding: spacing.lg,
        backgroundColor: colors.primary,
        gap: spacing.md,
    },
    heroGlow: {
        position: "absolute",
        width: 220,
        height: 220,
        borderRadius: 999,
        backgroundColor: "rgba(255,255,255,0.10)",
        top: -96,
        right: -72,
    },
    heroTopRow: {
        flexDirection: "row",
        alignItems: "center",
        gap: spacing.md,
    },
    avatarShell: {
        width: 76,
        height: 76,
        borderRadius: 38,
        alignItems: "center",
        justifyContent: "center",
        borderWidth: 2,
        borderColor: "rgba(255,255,255,0.35)",
        backgroundColor: "rgba(255,255,255,0.12)",
        padding: 3,
    },
    avatarPressed: {
        opacity: 0.88,
        transform: [{ scale: 0.97 }],
    },
    avatarCrop: {
        width: "100%",
        height: "100%",
        borderRadius: 999,
        overflow: "hidden",
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: PROFILE_PLACEHOLDER_BACKGROUND,
    },
    avatarImage: {
        width: "100%",
        height: "100%",
        borderRadius: 999,
    },
    avatarText: {
        color: PROFILE_PLACEHOLDER_TEXT,
        fontSize: 28,
        fontWeight: "800",
    },
    heroCopy: {
        flex: 1,
        gap: spacing.xs,
    },
    heroInfoButton: {
        alignSelf: "flex-start",
        borderRadius: 12,
        marginHorizontal: -spacing.xs,
        paddingHorizontal: spacing.xs,
        paddingVertical: 2,
    },
    heroInfoButtonPressed: {
        backgroundColor: "rgba(255,255,255,0.12)",
    },
    eyebrow: {
        color: "rgba(255,255,255,0.76)",
        fontSize: 12,
        fontWeight: "700",
        letterSpacing: 1.1,
        textTransform: "uppercase",
    },
    name: {
        color: "#ffffff",
        fontSize: 28,
        fontWeight: "800",
    },
    handle: {
        color: "rgba(255,255,255,0.82)",
        fontSize: 15,
        fontWeight: "500",
    },
    subtitle: {
        color: "rgba(255,255,255,0.9)",
        fontSize: 15,
        lineHeight: 24,
        maxWidth: 280,
    },
    badgeRow: {
        flexDirection: "row",
        flexWrap: "wrap",
        gap: spacing.sm,
    },
    badge: {
        borderRadius: 999,
        backgroundColor: "#d7f7e3",
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
    },
    badgeText: {
        color: colors.success,
        fontSize: 13,
        fontWeight: "700",
    },
    badgeSecondary: {
        borderRadius: 999,
        backgroundColor: "rgba(255,255,255,0.14)",
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
        borderWidth: 1,
        borderColor: "rgba(255,255,255,0.18)",
    },
    badgeSecondaryText: {
        color: "#ffffff",
        fontSize: 13,
        fontWeight: "700",
    },
    metricsGrid: {
        flexDirection: "row",
        flexWrap: "wrap",
        gap: spacing.md,
    },
    metricCard: {
        minHeight: 136,
        borderRadius: 20,
        padding: spacing.md,
        backgroundColor: colors.surface,
        borderWidth: 1,
        borderColor: colors.border,
        justifyContent: "space-between",
        boxShadow: "0px 6px 12px rgba(11, 31, 51, 0.04)",
        elevation: 1,
    },
    metricCardCompact: {
        minHeight: 96,
    },
    metricLabel: {
        color: colors.mutedText,
        fontSize: 13,
        fontWeight: "600",
    },
    metricValue: {
        color: colors.text,
        fontSize: 22,
        lineHeight: 30,
        fontWeight: "800",
    },
    metricValueCompact: {
        fontSize: 18,
        lineHeight: 24,
    },
    sectionCard: {
        borderRadius: spacing.lg,
        padding: spacing.lg,
        backgroundColor: colors.surface,
        gap: spacing.md,
    },
    sectionCardBottom: {
        borderBottomLeftRadius: 0,
        borderBottomRightRadius: 0,
    },
    sectionCardFlat: {
        borderRadius: 0,
    },
    sectionTitle: {
        color: colors.text,
        fontSize: 20,
        fontWeight: "800",
    },
    sectionHeader: {
        alignItems: "flex-start",
        flexDirection: "row",
        gap: spacing.md,
        justifyContent: "space-between",
    },
    sectionHeaderCopy: {
        flex: 1,
        gap: spacing.xs,
    },
    sectionDescription: {
        color: colors.mutedText,
        fontSize: 14,
        lineHeight: 24,
    },
    benefitsHero: {
        overflow: "hidden",
        paddingHorizontal: spacing.lg,
        paddingTop: spacing.xl,
        paddingBottom: spacing.lg,
        gap: spacing.sm,
    },
    benefitsHeroGlow: {
        position: "absolute",
        width: 240,
        height: 240,
        borderRadius: 999,
        backgroundColor: "rgba(255,255,255,0.12)",
        right: -96,
        top: -112,
    },
    benefitsHeroEyebrow: {
        color: "rgba(255,255,255,0.74)",
        fontSize: 12,
        fontWeight: "800",
        letterSpacing: 1.2,
        textTransform: "uppercase",
    },
    benefitsHeroTitle: {
        color: "#FFFFFF",
        fontSize: 30,
        lineHeight: 36,
        fontWeight: "900",
        maxWidth: 340,
    },
    benefitsHeroDescription: {
        color: "rgba(255,255,255,0.86)",
        fontSize: 15,
        lineHeight: 22,
        maxWidth: 360,
    },
    benefitsHeroMetrics: {
        flexDirection: "row",
        gap: spacing.sm,
        marginTop: spacing.sm,
    },
    benefitsHeroMetric: {
        flex: 1,
        minHeight: 84,
        borderRadius: 18,
        padding: spacing.md,
        justifyContent: "space-between",
        backgroundColor: "rgba(255,255,255,0.14)",
        borderWidth: 1,
        borderColor: "rgba(255,255,255,0.16)",
    },
    benefitsHeroMetricLabel: {
        color: "rgba(255,255,255,0.72)",
        fontSize: 12,
        fontWeight: "700",
    },
    benefitsHeroMetricValue: {
        color: "#FFFFFF",
        fontSize: 21,
        fontWeight: "900",
    },
    benefitsStatusPill: {
        alignSelf: "flex-start",
        borderRadius: 999,
        marginTop: spacing.xs,
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
        backgroundColor: "rgba(255,255,255,0.16)",
    },
    benefitsStatusPillText: {
        color: "#FFFFFF",
        fontSize: 12,
        fontWeight: "800",
    },
    benefitsSectionHeading: {
        alignItems: "flex-start",
        flexDirection: "row",
        gap: spacing.md,
    },
    benefitsNumberBadge: {
        minWidth: 48,
        height: 48,
        borderRadius: 16,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: colors.primaryMuted,
    },
    benefitsNumberBadgeText: {
        color: colors.primary,
        fontSize: 13,
        fontWeight: "900",
    },
    benefitsChoiceCard: {
        borderRadius: 20,
        padding: spacing.md,
        gap: spacing.sm,
        backgroundColor: colors.surfaceElevated,
        borderWidth: 1,
        borderColor: colors.borderSoft,
    },
    benefitsChoiceKicker: {
        color: colors.primary,
        fontSize: 12,
        fontWeight: "900",
        letterSpacing: 0.7,
        textTransform: "uppercase",
    },
    benefitsActivePromo: {
        alignItems: "center",
        flexDirection: "row",
        justifyContent: "space-between",
        gap: spacing.md,
        borderRadius: 18,
        padding: spacing.md,
        backgroundColor: colors.successMuted,
        borderWidth: 1,
        borderColor: colors.success,
    },
    benefitsActivePromoCopy: {
        flex: 1,
        gap: spacing.xs,
    },
    benefitsPromoCode: {
        color: colors.text,
        fontSize: 21,
        fontWeight: "900",
    },
    benefitsDiscountPill: {
        borderRadius: 999,
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
        backgroundColor: colors.surface,
    },
    benefitsDiscountPillText: {
        color: colors.success,
        fontSize: 14,
        fontWeight: "900",
    },
    benefitsInfoStrip: {
        borderRadius: 16,
        padding: spacing.md,
        backgroundColor: colors.primaryMuted,
    },
    benefitsInfoStripText: {
        color: colors.text,
        fontSize: 14,
        lineHeight: 21,
        fontWeight: "600",
    },
    benefitsSoftAction: {
        alignSelf: "stretch",
        minHeight: 46,
        borderRadius: 14,
        alignItems: "center",
        justifyContent: "center",
        paddingHorizontal: spacing.md,
        backgroundColor: colors.primaryMuted,
    },
    benefitsSoftActionText: {
        color: colors.primary,
        fontSize: 14,
        fontWeight: "800",
        textAlign: "center",
    },
    benefitsGuideStack: {
        gap: spacing.sm,
    },
    benefitsGuideCard: {
        borderRadius: 20,
        padding: spacing.md,
        gap: spacing.md,
        backgroundColor: colors.surfaceElevated,
        borderWidth: 1,
        borderColor: colors.borderSoft,
    },
    benefitsGuideHeader: {
        alignItems: "flex-start",
        flexDirection: "row",
        gap: spacing.md,
    },
    benefitsGuideIndex: {
        width: 34,
        height: 34,
        borderRadius: 12,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: colors.primary,
    },
    benefitsGuideIndexText: {
        color: "#FFFFFF",
        fontSize: 14,
        fontWeight: "900",
    },
    benefitsRuleText: {
        color: colors.stateText,
        fontSize: 13,
        lineHeight: 20,
        fontWeight: "600",
    },
    benefitsScaleRow: {
        flexDirection: "row",
        flexWrap: "wrap",
        gap: spacing.sm,
    },
    benefitsScaleItem: {
        flexGrow: 1,
        minWidth: 64,
        borderRadius: 14,
        alignItems: "center",
        paddingHorizontal: spacing.sm,
        paddingVertical: spacing.sm,
        backgroundColor: colors.surface,
        borderWidth: 1,
        borderColor: colors.border,
    },
    benefitsScalePercent: {
        color: colors.primary,
        fontSize: 16,
        fontWeight: "900",
    },
    benefitsScaleAmount: {
        color: colors.mutedText,
        fontSize: 11,
        fontWeight: "700",
    },
    benefitsProgressHeader: {
        alignItems: "flex-end",
        flexDirection: "row",
        justifyContent: "space-between",
        gap: spacing.md,
    },
    benefitsProgressTarget: {
        color: colors.mutedText,
        fontSize: 13,
        fontWeight: "800",
    },
    benefitsProgressTrack: {
        height: 10,
        overflow: "hidden",
        borderRadius: 999,
        backgroundColor: colors.surfaceMuted,
    },
    benefitsProgressFill: {
        height: "100%",
        borderRadius: 999,
        backgroundColor: colors.primary,
    },
    statusBadge: {
        borderRadius: 999,
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
    },
    statusBadgeSuccess: {
        backgroundColor: "#dff7e8",
    },
    statusBadgeMuted: {
        backgroundColor: colors.surfaceMuted,
    },
    statusBadgeText: {
        fontSize: 12,
        fontWeight: "800",
        textTransform: "uppercase",
        letterSpacing: 0.6,
    },
    statusBadgeTextSuccess: {
        color: colors.success,
    },
    statusBadgeTextMuted: {
        color: colors.mutedText,
    },
    detailRow: {
        gap: spacing.xs,
    },
    detailStack: {
        gap: spacing.md,
    },
    detailRowButton: {
        borderRadius: 16,
        marginHorizontal: -spacing.sm,
        paddingHorizontal: spacing.sm,
        paddingVertical: spacing.sm,
    },
    detailRowButtonPressed: {
        backgroundColor: colors.primaryMuted,
    },
    detailLabel: {
        color: colors.mutedText,
        fontSize: 13,
        fontWeight: "700",
        textTransform: "uppercase",
        letterSpacing: 0.6,
    },
    detailValue: {
        color: colors.text,
        fontSize: 17,
        fontWeight: "600",
    },
    detailDivider: {
        height: 1,
        backgroundColor: colors.border,
    },
    preferencesChipRow: {
        flexDirection: "row",
        flexWrap: "wrap",
        gap: spacing.sm,
    },
    preferenceChip: {
        borderRadius: 999,
        borderWidth: 1,
        borderColor: colors.borderSoft,
        backgroundColor: colors.surfaceElevated,
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
    },
    preferenceChipActive: {
        borderColor: colors.primary,
        backgroundColor: colors.primaryMuted,
    },
    preferenceChipPressed: {
        opacity: 0.9,
    },
    preferenceChipText: {
        color: colors.text,
        fontSize: 13,
        fontWeight: "700",
    },
    preferenceChipTextActive: {
        color: colors.primary,
    },
    themeModeRow: {
        marginTop: spacing.xs,
    },
    themeModeChip: {
        alignSelf: "flex-start",
    },
    colorSwatchRow: {
        flexDirection: "row",
        flexWrap: "wrap",
        gap: spacing.sm,
    },
    colorSwatchShell: {
        width: 34,
        height: 34,
        borderRadius: 999,
        borderWidth: 1,
        borderColor: colors.borderSoft,
        padding: 4,
        backgroundColor: colors.surface,
    },
    colorSwatchShellActive: {
        borderColor: colors.text,
    },
    colorSwatch: {
        width: "100%",
        height: "100%",
        borderRadius: 999,
    },
    loadingBox: {
        alignItems: "center",
        justifyContent: "center",
        minHeight: 72,
    },
    subsection: {
        gap: spacing.sm,
    },
    subsectionTitle: {
        color: colors.text,
        fontSize: 16,
        fontWeight: "800",
    },
    emptyHint: {
        color: colors.mutedText,
        fontSize: 14,
        lineHeight: 22,
    },
    tagList: {
        gap: spacing.sm,
    },
    tag: {
        borderRadius: 18,
        borderWidth: 1,
        borderColor: colors.border,
        backgroundColor: colors.surfaceSoft,
        gap: 4,
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
    },
    tagTitle: {
        color: colors.text,
        fontSize: 15,
        fontWeight: "700",
    },
    tagSubtitle: {
        color: colors.mutedText,
        fontSize: 13,
        lineHeight: 18,
    },
    formGroup: {
        gap: spacing.sm,
    },
    formLabel: {
        color: colors.text,
        fontSize: 14,
        fontWeight: "700",
    },
    formInput: {
        backgroundColor: colors.surface,
        borderColor: colors.border,
        borderRadius: 14,
        borderWidth: 1,
        color: colors.text,
        fontSize: 16,
        minHeight: 50,
        paddingHorizontal: spacing.md,
    },
    formHint: {
        color: colors.mutedText,
        fontSize: 13,
        lineHeight: 21,
    },
    errorBox: {
        borderRadius: 16,
        borderWidth: 1,
        borderColor: "#fecdd3",
        backgroundColor: colors.dangerMuted,
        gap: spacing.sm,
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
    },
    errorText: {
        color: colors.danger,
        fontSize: 14,
        lineHeight: 20,
    },
    primaryActionButton: {
        minHeight: 50,
        borderRadius: 16,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: colors.primary,
        paddingHorizontal: spacing.md,
    },
    primaryActionButtonPressed: {
        opacity: 0.84,
    },
    primaryActionButtonDisabled: {
        opacity: 0.5,
    },
    primaryActionButtonText: {
        color: "#ffffff",
        fontSize: 16,
        fontWeight: "800",
    },
    secondaryInlineButton: {
        alignSelf: "flex-start",
        borderRadius: 12,
        borderWidth: 1,
        borderColor: "#f0a3b1",
        paddingHorizontal: spacing.md,
        paddingVertical: spacing.sm,
    },
    secondaryInlineButtonPressed: {
        opacity: 0.8,
    },
    secondaryInlineButtonText: {
        color: colors.danger,
        fontSize: 14,
        fontWeight: "700",
    },
    discountStack: {
        gap: spacing.sm,
    },
    discountRow: {
        alignItems: "center",
        backgroundColor: colors.surfaceElevated,
        borderColor: colors.borderSoft,
        borderRadius: 16,
        borderWidth: 1,
        flexDirection: "row",
        gap: spacing.md,
        justifyContent: "space-between",
        padding: spacing.sm,
    },
    discountRowPressed: {
        opacity: 0.86,
        transform: [{ scale: 0.99 }],
    },
    discountImage: {
        width: 64,
        height: 64,
        borderRadius: 13,
        backgroundColor: colors.surfaceMuted,
    },
    discountCopy: {
        flex: 1,
        gap: 3,
        minWidth: 0,
    },
    discountTitle: {
        color: colors.text,
        fontSize: 15,
        fontWeight: "800",
    },
    discountCode: {
        color: colors.mutedText,
        fontSize: 12,
        fontWeight: "700",
    },
    discountValue: {
        color: "#9A3412",
        fontSize: 14,
        fontWeight: "900",
    },
    discountBadge: {
        borderRadius: 999,
        backgroundColor: "#FFEDD5",
        paddingHorizontal: spacing.sm,
        paddingVertical: 6,
    },
    discountArrow: {
        color: colors.primary,
        fontSize: 20,
        fontWeight: "800",
    },
    historyCardButton: {
        borderRadius: spacing.lg,
        overflow: "hidden",
    },
    historyCardButtonPressed: {
        opacity: 0.88,
        transform: [{ scale: 0.99 }],
    },
    historyCardHeader: {
        alignItems: "center",
        flexDirection: "row",
        gap: spacing.md,
        justifyContent: "space-between",
    },
    historyCardCopy: {
        flex: 1,
        gap: spacing.xs,
    },
    historyCardTitle: {
        color: colors.text,
        fontSize: 20,
        fontWeight: "800",
    },
    historyCardSubtitle: {
        color: colors.mutedText,
        fontSize: 14,
        lineHeight: 22,
    },
    historyCardArrow: {
        color: colors.primary,
        fontSize: 24,
        fontWeight: "700",
        lineHeight: 24,
    },
    signOutButton: {
        minHeight: 50,
        borderRadius: 16,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: colors.primary,
        paddingHorizontal: spacing.md,
    },
    signOutButtonPressed: {
        opacity: 0.84,
    },
    signOutButtonText: {
        color: "#ffffff",
        fontSize: 16,
        fontWeight: "800",
    },
    deleteAccountButton: {
        minHeight: 50,
        borderRadius: 16,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: "#dc2626",
        paddingHorizontal: spacing.md,
    },
    deleteAccountButtonPressed: {
        opacity: 0.84,
    },
    deleteAccountButtonDisabled: {
        opacity: 0.5,
    },
    deleteAccountButtonText: {
        color: "#ffffff",
        fontSize: 16,
        fontWeight: "800",
    },
    avatarViewerBackdrop: {
        flex: 1,
        backgroundColor: "rgba(4, 10, 18, 0.92)",
        alignItems: "center",
        justifyContent: "center",
        padding: spacing.lg,
    },
    avatarViewerDismissArea: {
        ...StyleSheet.absoluteFillObject,
    },
    avatarViewerContent: {
        width: "100%",
        alignItems: "center",
        gap: spacing.lg,
        zIndex: 1,
    },
    avatarViewerFrame: {
        width: 280,
        height: 280,
        borderRadius: 999,
        overflow: "hidden",
        borderWidth: 3,
        borderColor: "rgba(255,255,255,0.16)",
        backgroundColor: PROFILE_PLACEHOLDER_BACKGROUND,
        alignItems: "center",
        justifyContent: "center",
    },
    avatarViewerImage: {
        width: "100%",
        height: "100%",
        borderRadius: 999,
    },
    avatarViewerPlaceholder: {
        width: "100%",
        height: "100%",
        borderRadius: 999,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: PROFILE_PLACEHOLDER_BACKGROUND,
    },
    avatarViewerPlaceholderText: {
        color: PROFILE_PLACEHOLDER_TEXT,
        fontSize: 88,
        fontWeight: "800",
    },
    avatarViewerName: {
        color: "#ffffff",
        fontSize: 24,
        fontWeight: "800",
        textAlign: "center",
    },
    avatarViewerActions: {
        width: "100%",
        maxWidth: 320,
        gap: spacing.sm,
    },
    avatarViewerPrimaryAction: {
        minHeight: 50,
        borderRadius: 16,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: colors.surface,
        paddingHorizontal: spacing.md,
    },
    avatarViewerPrimaryActionText: {
        color: colors.primary,
        fontSize: 16,
        fontWeight: "800",
    },
    avatarViewerDestructiveAction: {
        minHeight: 50,
        borderRadius: 16,
        alignItems: "center",
        justifyContent: "center",
        backgroundColor: "#d74b5c",
        paddingHorizontal: spacing.md,
    },
    avatarViewerDestructiveActionText: {
        color: "#ffffff",
        fontSize: 16,
        fontWeight: "800",
    },
    avatarViewerActionPressed: {
        opacity: 0.82,
    },
    avatarViewerActionDisabled: {
        opacity: 0.42,
    },
})
