...
from src.normalize import lower_optional_str, normalize_email, normalize_phone, optional_str
...
def _website_profile_phone(profile: dict[str, Any]) -> str | None:
    return normalize_phone(profile.get("personal_phone")) or normalize_phone(profile.get("personal_mobile"))


def _website_placeholder_phone(*, website_user_id: int) -> str:
    return f"+99{website_user_id % 1000000000000:012d}"
...
async def sync_local_user_from_website_payload(db: AsyncSession, user: User, payload: dict[str, Any]) -> User:
    ...
    next_phone_number = _website_profile_phone(profile)
    next_email = normalize_email(profile.get("email"))

    if next_phone_number and next_phone_number != user.phone_number:
        existing_by_phone = await get_user_by_phone_number(db, next_phone_number)
        if existing_by_phone is not None and existing_by_phone.id != user.id:
            next_phone_number = None
    ...
    if next_email is not None:
        user_update_data["email"] = next_email
    if next_phone_number is not None:
        user_update_data["phone_number"] = next_phone_number
...
async def create_local_user_from_website_payload(db: AsyncSession, payload: dict[str, Any]) -> User:
    ...
    phone_number = _website_profile_phone(profile) or _website_placeholder_phone(website_user_id=website_user_id)
...
async def resolve_user_for_website_login(db: AsyncSession, payload: dict[str, Any]) -> User:
    ...
    phone_number = _website_profile_phone(profile)
    if phone_number:
        ...
        if user is not None:
            ...
            if existing_user_identity is not None and existing_user_identity.website_user_id != website_user_id:
...
async def refresh_linked_website_identity(db: AsyncSession, *, website_identity: WebsiteIdentity, payload: dict[str, Any]) -> WebsiteIdentity:
    ...
    if user is None:
        raise RuntimeError(f"Linked user {website_identity.user_id} was not found")
