...
def estimate_discount_amount(
    *, subtotal: Decimal, calculation_mode: str, discount_percent: Decimal | None, discount_amount: Decimal | None
) -> Decimal | None:
    if calculation_mode == "percent":
        return percent_to_amount(subtotal, discount_percent)
    if calculation_mode == "fixed_amount":
        return fixed_to_amount(subtotal, discount_amount)
    return None
...
def preferred_currency(
    *, requested_currency: str | None, available_options: list[ResolvedDiscountOption], bonus_option: dict | None
) -> str | None:
    normalized_requested_currency = optional_str(requested_currency)
    if normalized_requested_currency:
        return normalized_requested_currency
    for option in available_options:
        if option.currency:
            return option.currency
    if bonus_option and bonus_option.get("currency"):
        return bonus_option["currency"]
    return None
