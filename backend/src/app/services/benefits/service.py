...
async def _resolve_subtotal(db: AsyncSession, *, user_id: int, explicit_subtotal: Decimal | None) -> tuple[Decimal, str]:
    if explicit_subtotal is not None:
        return quantize_money(explicit_subtotal) or Decimal("0.00"), "request"
    basket = await get_basket_by_user_id(db, user_id)
    if basket is None:
        return Decimal("0.00"), "basket"
    total = Decimal("0.00")
...
def _resolve_bonus_option(
    *, bonus_account: WebsiteBonusAccount | None, subtotal: Decimal, requested_bonus_amount: Decimal | None
) -> dict | None:
    ...
    if not bonus_account.is_active:
        ...
        if requested_amount > max_applicable_amount:
            reason = "Requested bonus amount was capped by the available website balance or subtotal"

    return {
        "status": status,
        "is_available": is_available,
        "source_record_id": bonus_account.id,
        "balance": balance,
        "currency": bonus_account.currency,
        "max_applicable_amount": max_applicable_amount,
        "requested_amount": requested_amount,
        "applicable_amount": applicable_amount,
        "estimated_total_after_bonus": quantize_money(max(Decimal("0.00"), subtotal - applicable_amount)) or Decimal("0.00"),
        "reason": reason,
    }
...
async def resolve_benefits_for_user(
    ...
    *,
    ...
    if website_identity is not None:
        ...
        entitlement_options = [
            build_website_entitlement_option(entitlement, subtotal=effective_subtotal, now=now)
            for entitlement in website_identity.discount_entitlements
        ]
    ...
    if normalized_code:
        ...
        if app_promo is not None:
            code_matches.append(
                await build_app_promo_option(db, app_promo=app_promo, subtotal=effective_subtotal, user_id=user.id, now=now)
            )
    ...
    bonus_option = _resolve_bonus_option(
        bonus_account=website_identity.bonus_account_snapshot if website_identity is not None else None,
    ...
    if trimmed_code and not code_matches:
        unresolved_code_reason = "Promo code was not found in website coupons or app promos"
    ...
    return {
        ...
        "currency": preferred_currency(
            requested_currency=currency, available_options=available_discount_options, bonus_option=bonus_option
        ),
