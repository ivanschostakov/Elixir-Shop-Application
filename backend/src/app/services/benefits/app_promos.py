...
def resolve_app_promo_discount(app_promo: AppPromo) -> tuple[str, Decimal | None, Decimal | None, str | None]:
    ...
    if benefit_kind in PERCENT_BENEFIT_KINDS or (app_promo.discount_percent is not None and app_promo.discount_amount is None):
        return "percent", quantize_money(app_promo.discount_percent), None, None
    if benefit_kind in FIXED_BENEFIT_KINDS or app_promo.discount_amount is not None:
        return "fixed_amount", None, quantize_money(app_promo.discount_amount), None
