...
def _offer_min_unix(offer: dict[str, Any]) -> int:
    try: return int((offer.get("offer_details") or {}).get("delivery_interval", {}).get("min"))
    except (TypeError, ValueError): return 10**18
...
def _extract_cdek_provider_ref(response: dict[str, Any], order: Order) -> str:
    ...
    if not provider_ref and requests and isinstance(requests[0], dict): provider_ref = requests[0].get("request_uuid")
...
async def create_delivery_for_order(order: Order) -> dict[str, Any]:
    if order.delivery_created_at is not None: return {}
    service = (order.selected_delivery_service or "").strip().upper()
    if service == "YANDEX": return await _create_yandex_delivery(order)
    if service == "CDEK": return await _create_cdek_delivery(order)
