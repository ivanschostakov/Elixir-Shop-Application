...
async def read_upload_file_limited(upload: UploadFile, *, max_bytes: int, label: str) -> bytes:
    ...
    while True:
        ...
        if not chunk: break
        if len(content) + len(chunk) > max_bytes: raise HTTPException(status_code=status.HTTP_413_CONTENT_TOO_LARGE, detail=f"{label} exceeds {format_upload_size_limit(max_bytes)}")
