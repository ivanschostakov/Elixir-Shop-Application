...
def build_avatar_path(user_id: int) -> Path:
    return AVATARS_MEDIA_DIR / f"{user_id}{AVATAR_EXTENSION}"
...
def validate_avatar_content_type(content_type: str | None) -> None:
    if (content_type or "").lower() not in ALLOWED_AVATAR_TYPES:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Only JPEG, PNG, and WEBP images are supported")
...
def validate_avatar_content(content: bytes) -> None:
    ...
    if len(content) > MAX_AVATAR_SIZE_BYTES:
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Avatar image must be 5 MB or smaller")
...
def convert_avatar_to_png(content: bytes) -> bytes:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Uploaded file is not a valid image") from error
