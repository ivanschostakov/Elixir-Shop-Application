...
def build_review_attachment_url(request: Request, image_path: Path) -> str:
    ...
    except ValueError: relative_path = f"reviews/{image_path.name}"
    return f"{base_url}/media/{relative_path}?v={version}"


def validate_review_attachments_count(attachments_count: int) -> None:
    if attachments_count > MAX_REVIEW_IMAGES_COUNT: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"You can upload up to {MAX_REVIEW_IMAGES_COUNT} images per review")
...
def validate_review_attachment(content: bytes, *, mime_type: str | None) -> str:
    ...
    if normalized_mime_type not in ALLOWED_REVIEW_IMAGE_TYPES: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Only JPEG, PNG, and WEBP review images are supported")
    if not content: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Uploaded review image is empty")
    if len(content) > MAX_REVIEW_IMAGE_SIZE_BYTES: raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Each review image must be 8 MB or smaller")
    return normalized_mime_type


def validate_review_attachments_total_size(total_size_bytes: int) -> None:
    if total_size_bytes > MAX_REVIEW_TOTAL_SIZE_BYTES: raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="Total review images size must be 24 MB or smaller")


def build_review_attachment_filename(mime_type: str) -> str: return f"{uuid4().hex}{_MIME_EXTENSION.get(mime_type, '.jpg')}"
