...
async def create_admin_invitation(
    ...
    existing_user = (
        await db.execute(
            select(AdminIdentity).where(AdminIdentity.email == email)
        )
    ).scalar_one_or_none()
...
async def resend_admin_invitation(
    ...
    inviter = (await db.execute(
        select(AdminIdentity).where(
            AdminIdentity.id == invitation.invited_by_user_id
        )
...
async def preview_admin_invitation(
    ...
    existing_user = (await db.execute(
        select(AdminIdentity.id).where(
            AdminIdentity.email == invitation.email
        )
...
async def accept_admin_invitation(
    ...
    user = (await db.execute(
        select(AdminIdentity)
        .where(AdminIdentity.email == invitation.email)
        .with_for_update()
    )).scalar_one_or_none()
    if user is None:
