...
async def product_categories_get(
    ...
    if cached_items is not None:
        return [ProductCategoryRead.model_validate(item) for item in cached_items]

    items = [
        ProductCategoryRead.model_validate(category)
        for category in await get_product_categories(
            db,
            q=normalized_q,
            name=normalized_name,
            offset=offset,
    ...
    await cache.set_json(
        ...
        ttl_seconds=PRODUCT_CATEGORIES_CACHE_TTL_SECONDS,
        key_prefix="product-categories:list",
    )
    return items
