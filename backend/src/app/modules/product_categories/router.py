...
async def product_categories_get(q: str | None = Query(default=None, min_length=1, max_length=100), name: str | None = Query(default=None, min_length=1, max_length=200), limit: int = Query(default=100, ge=1, le=100), offset: int = Query(default=0, ge=0), sort: Literal["app_order", "newest", "name_asc", "name_desc"] | None = Query(default="app_order"), db: AsyncSession = Depends(get_db)) -> list[ProductCategoryRead]:
    normalized_q = q.strip().lower() if q is not None else None
    normalized_name = name.strip() if name is not None else None
    # Installed app builds used name_asc before admin-controlled ordering existed.
    # Keep those builds on the authoritative app order without requiring an OTA.
    effective_sort = "app_order" if sort in {None, "name_asc"} else sort
    ...
    items = [ProductCategoryRead.model_validate(category)  for category in await get_product_categories(db, q=normalized_q, name=normalized_name, offset=offset, limit=limit, sort=effective_sort)]
