...
async def create_my_basket_item(
    payload: BasketItemCreate, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
) -> BasketRead:
...
async def update_my_basket_item(
    item_id: int,
...
async def delete_my_basket_item(
    item_id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
) -> BasketRead:
    basket = await _ensure_basket(db, current_user.id)
    basket_item = await get_basket_item_by_id(db, item_id, basket_id=basket.id, user_id=current_user.id)
    if basket_item is None:
