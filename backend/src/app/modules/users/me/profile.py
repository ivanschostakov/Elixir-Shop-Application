...
async def update_my_personal_data(payload: PersonalDataUpdatePayload, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> AuthUserRead:
    if payload.username is not None and payload.username != current_user.username:
        ...
        if username_user is not None and username_user.id != current_user.id: raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User with this username already exists")
    ...
    if "phone_number" in payload.model_fields_set: current_user.phone_number = payload.phone_number
