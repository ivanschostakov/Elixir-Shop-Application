...
async def upsert_my_push_token(payload: UserPushTokenUpsert, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> UserPushTokenRead: return UserPushTokenRead.model_validate(await upsert_user_push_token(db, user_id=current_user.id, data=payload, commit=True))


@push_tokens_router.delete("", response_model=UserPushTokenDeleteResponse, status_code=status.HTTP_200_OK)
async def delete_my_push_token(payload: UserPushTokenDelete, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> UserPushTokenDeleteResponse:
