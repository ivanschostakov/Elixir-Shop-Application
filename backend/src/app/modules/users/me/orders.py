...
async def create_my_order(payload: CreateOrderPayload, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user), _app_integrity: None = Depends(require_app_integrity("orders:create"))) -> OrderRead:
    order = await create_order_from_draft_for_user(db, request=request, user=current_user, draft_id=payload.draft_id, payment_method=payload.payment_method, entered_code=payload.code, use_bonus_rubles=payload.use_bonus_rubles, reward_mode=payload.reward_mode) if payload.draft_id is not None else await create_order_from_basket_for_user(db, request=request, user=current_user, payment_method=payload.payment_method, entered_code=payload.code, use_bonus_rubles=payload.use_bonus_rubles, reward_mode=payload.reward_mode)
...
async def list_my_orders(request: Request, limit: int = Query(default=20, ge=1, le=100), offset: int = Query(default=0, ge=0), history_bucket: OrderHistoryBucket | None = Query(default=None), status_code: OrderStatusCode | None = Query(default=None), created_from: datetime | None = Query(default=None), created_to: datetime | None = Query(default=None), db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user), _app_integrity: None = Depends(require_app_integrity("orders:list"))) -> list[OrderRead]:
...
async def repeat_my_order(order_id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user), _app_integrity: None = Depends(require_app_integrity("orders:repeat"))) -> OrderDraftRead:
    draft = await repeat_order_as_draft_for_user(db, user_id=current_user.id, order_id=order_id)
    if draft is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Order not found")
    return await serialize_order_draft(request, db, draft)


@my_orders_router.get("/{order_id}", response_model=OrderRead)
async def get_my_order(order_id: int, request: Request, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user), _app_integrity: None = Depends(require_app_integrity("orders:read"))) -> OrderRead:
