...
async def create_my_order_draft(
...
async def get_my_latest_order_draft(
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> OrderDraftRead:
...
async def get_my_order_draft_options(
    draft_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
async def update_my_order_draft(
    draft_id: int,
...
async def delete_my_order_draft(
    draft_id: int,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> None:
