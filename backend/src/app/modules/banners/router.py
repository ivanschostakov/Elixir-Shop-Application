...
async def banners_get(
    limit: int = Query(default=10, ge=1, le=50),
    offset: int = Query(default=0, ge=0),
    sort: Literal["newest", "priority_desc", "priority_asc"] | None = Query(default="priority_desc"),
    ...
    if cached_items is not None:
        return [BannerRead.model_validate(item) for item in cached_items]

    items = [
        BannerRead.model_validate(banner)
        for banner in await get_banners(
            db,
            offset=offset,
            limit=limit,
            sort=sort,
    ...
    await cache.set_json(
        ...
        ttl_seconds=BANNERS_CACHE_TTL_SECONDS,
        key_prefix="banners:list",
    )
    return items
