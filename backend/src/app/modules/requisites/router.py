...
async def requisites_create(data: RequisiteCreate, db: AsyncSession = Depends(get_db), _: AdminIdentity = Depends(get_current_admin_user)) -> RequisiteRead: return RequisiteRead.model_validate(await create_requisite(db, data))


@requisites_router.patch("/{requisite_id}", response_model=RequisiteRead)
async def requisites_patch(requisite_id: int, data: RequisiteUpdate, db: AsyncSession = Depends(get_db), _: AdminIdentity = Depends(get_current_admin_user)) -> RequisiteRead:
    requisite = await get_requisite_by_id(db, requisite_id)
    if requisite is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Requisite not found")
...
async def requisites_delete(requisite_id: int, db: AsyncSession = Depends(get_db), _: AdminIdentity = Depends(get_current_admin_user)) -> None:
    requisite = await get_requisite_by_id(db, requisite_id)
    if requisite is None: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Requisite not found")
