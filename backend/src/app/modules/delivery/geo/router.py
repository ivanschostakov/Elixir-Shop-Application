...
def _normalize_ll_key(ll: str) -> str:
    ...
    if len(raw_parts) != 2: return ll.strip()
    try:
        lon = round(float(raw_parts[0]), 4)
        lat = round(float(raw_parts[1]), 4)

    except ValueError: return ll.strip()
    return f"{lon:.4f},{lat:.4f}"


@geo_router.get("/suggest", response_model=list[GeoSuggestResult], status_code=status.HTTP_200_OK)
async def suggest(query: str = Query(..., min_length=1, description="User search text"), ll: str = Query(..., description="Coordinates in format 'lon,lat'"), lang: str = Query("ru_RU"), version: int = Query(9, ge=1, alias="v"), geo: GeoClient = Depends(get_geo_client)) -> list[GeoSuggestResult]:
    cache = get_cache_service()
    base_key = build_cache_key(route="delivery:geo:suggest", params={
    ...
    if cached_items is not None: return [GeoSuggestResult.model_validate(item) for item in cached_items]

    items = await geo.geosuggest(text=query, ll=ll, lang=lang, v=version)
    await cache.set_json(cache_key, [item.model_dump(mode="json") for item in items], ttl_seconds=GEO_SUGGEST_CACHE_TTL_SECONDS, key_prefix="delivery:geo:suggest")
    return items


@geo_router.get("/code", response_model=GeoCodeResult, status_code=status.HTTP_200_OK)
async def code(address: str = Query(..., min_length=1), uri: str | None = Query(None), lang: str = Query("ru_RU"), results: int = Query(1, ge=1, le=10), geo: GeoClient = Depends(get_geo_client)) -> GeoCodeResult:
