...
def normalize_product_text(value: str | None) -> str | None:
    ...
    for paragraph in paragraphs:
        ...
        if not cleaned:
            continue
        if normalized and _should_merge_paragraphs(normalized[-1], cleaned):
            normalized[-1] = _merge_chunks(normalized[-1], cleaned)
        else:
...
def _should_keep_linebreak(left: str, right: str) -> bool:
    if _looks_like_list_item(right):
        return True
    if _looks_like_list_item(left):
        return left.endswith(":")
    if left.endswith(":") and not right.startswith(("—", "–", "-", ",", ";", ":")):
        return True
    return False
...
def _should_merge_paragraphs(left: str, right: str) -> bool:
    right = right.lstrip()
    if not right or _looks_like_list_item(left) or _looks_like_list_item(right):
        return False
    if right[0] in _ATTACHED_PUNCTUATION or right[0] in "—–-([{":
        return True
    if right[0].islower():
        return True
...
def _merge_chunks(left: str, right: str) -> str:
    ...
    if _should_join_without_space(left, right):
        separator = ""
    elif right[0] in _ATTACHED_PUNCTUATION:
        separator = ""
    else:
        separator = " "
...
def _should_join_without_space(left: str, right: str) -> bool:
    ...
    if len(right_fragment) == 1 and len(left_fragment) >= 4:
        return True
...
def _looks_like_heading(text: str) -> bool:
    ...
    compact = text.strip()
    if not compact or _looks_like_list_item(compact):
        return False
    if compact.endswith(":"):
        return True
    if any(mark in compact for mark in _SENTENCE_ENDINGS):
        return False
...
def _ends_cleanly(text: str) -> bool:
    return text.rstrip().endswith(tuple(_SENTENCE_ENDINGS))
