...
def _coupon_key_from_model(coupon: WebsiteCoupon) -> tuple[str, str]:
    if coupon.website_coupon_external_id is not None:
        return "external_id", str(coupon.website_coupon_external_id)
...
async def upsert_website_referral_profile(session: AsyncSession, *, website_identity_id: int, data: dict[str, Any] | None) -> None:
    existing = (
        await session.execute(select(WebsiteReferralProfile).where(WebsiteReferralProfile.website_identity_id == website_identity_id))
    ).scalar_one_or_none()
    if data is None:
...
async def upsert_website_bonus_account(
    session: AsyncSession, *, website_identity_id: int, data: dict[str, Any] | None, last_synced_at
) -> None:
    existing = (
        await session.execute(select(WebsiteBonusAccount).where(WebsiteBonusAccount.website_identity_id == website_identity_id))
    ).scalar_one_or_none()
    if data is None:
        if existing is None:
            return
...
async def sync_website_discount_entitlements(
    session: AsyncSession, *, website_identity_id: int, rows: list[dict[str, Any]], last_synced_at
) -> None:
    existing_rows = list(
        (
            await session.execute(
                select(WebsiteDiscountEntitlement).where(WebsiteDiscountEntitlement.website_identity_id == website_identity_id)
            )
        ).scalars()
    )
    existing_by_key: dict[tuple[str, str, str], WebsiteDiscountEntitlement] = {}
    for existing in existing_rows:
        existing_by_key.setdefault(_entitlement_key_from_model(existing), existing)
...
async def sync_website_coupon_snapshots(
    session: AsyncSession, *, website_identity_id: int, rows: list[dict[str, Any]], last_synced_at
) -> None:
    existing_rows = list(
        (await session.execute(select(WebsiteCoupon).where(WebsiteCoupon.website_identity_id == website_identity_id))).scalars()
    )
    existing_by_key: dict[tuple[str, str], WebsiteCoupon] = {}
    for existing in existing_rows:
        existing_by_key.setdefault(_coupon_key_from_model(existing), existing)
