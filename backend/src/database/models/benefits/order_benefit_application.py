...
from src.database.limits import CURRENCY_CODE_MAX_LENGTH, EXTERNAL_ID_MAX_LENGTH, PROMO_CODE_MAX_LENGTH, SOURCE_KIND_MAX_LENGTH, STATUS_MAX_LENGTH
...
class OrderBenefitApplication(Base, IdPkMixin, TimestampMixin):
    ...
    source_external_id: Mapped[str | None] = mapped_column(String(length=EXTERNAL_ID_MAX_LENGTH), nullable=True, index=True)
    ...
    benefit_units: Mapped[Decimal | None] = mapped_column(Numeric(14, 2), nullable=True)
    external_reference: Mapped[str | None] = mapped_column(String(length=EXTERNAL_ID_MAX_LENGTH), nullable=True, index=True)
