...
class OrderDraftItem(Base, IdPkMixin, TimestampMixin):
    __tablename__ = "order_draft_items"

    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    draft_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("order_drafts.id", ondelete="CASCADE"), nullable=False, index=True)
