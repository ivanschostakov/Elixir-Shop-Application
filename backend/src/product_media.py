...
def product_image_path(product_id: int | None, system_id) -> Path | None:
    ...
    if directory is None or system_id_str is None: return None
...
def resolve_product_image_path(*, product_id: int | None, system_id) -> Path | None:
    ...
    if current_path is not None and current_path.exists(): return current_path
    return None
...
def products_media_relative_path(image_path: Path) -> str:
    try: return image_path.relative_to(PRODUCTS_MEDIA_DIR).as_posix()
    except ValueError: return image_path.name


def build_products_media_url(base_url: str, image_path: Path | None) -> str:
    base = (PUBLIC_API_BASE_URL or base_url).rstrip("/")
    if image_path is None or not image_path.exists(): return f"{base}/media/products/{PRODUCT_IMAGE_PLACEHOLDER}"
