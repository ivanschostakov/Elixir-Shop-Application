...
async def _run_once() -> None:
    stats = await run_sync(
        website_user_ids=[],
        user_ids=[],
        limit=(WEBSITE_IDENTITY_SYNC_LIMIT if WEBSITE_IDENTITY_SYNC_LIMIT > 0 else None),
        batch_size=max(WEBSITE_IDENTITY_SYNC_BATCH_SIZE, 1),
        stale_minutes=max(WEBSITE_IDENTITY_SYNC_STALE_MINUTES, 0),
        dry_run=WEBSITE_IDENTITY_SYNC_DRY_RUN,
    )
    log.info(
        "website identity sync tick completed scanned=%s synced=%s missing_remote=%s failed=%s skipped=%s",
        stats.scanned,
        stats.synced,
    ...
    try:
        while not stop_event.is_set():
            try:
                ...
                log.exception("website identity sync tick failed")
