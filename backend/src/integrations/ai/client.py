...
class ProfessorClient(AsyncClient):
    ...
    def _build_tools(model: BotModel, function_tools: list[dict[str, Any]] | None = None) -> list[Any]:
        ...
        tools: list[Any] = [FileSearchToolParam(type="file_search", vector_store_ids=[vector_store_id])]
        if model == BotModel.PREMIUM: tools += [CodeInterpreter(container=CodeInterpreterContainerCodeInterpreterToolAuto(type="auto"),type="code_interpreter"), ImageGeneration(type="image_generation")]
        if function_tools: tools.extend(function_tools)
        return tools
    ...
    async def _extract_v2_files(self, response: Response, started_at: int) -> list[dict[str, Any]]:
        ...
        for idx, item in enumerate(getattr(response, "output", []) or []):
            ...
            async for file in self.containers.files.list(container_id=container_id, order="desc", limit=100):
                ...
                if not file_id or file_id in seen_file_ids: continue
                seen_file_ids.add(file_id)

                file_created_at = int(getattr(file, "created_at", 0) or 0)
                if file_created_at and file_created_at < started_at: break

                file_path = str(getattr(file, "path", "") or "")
                if file_path and not file_path.startswith("/mnt/data/"): continue
                ...
                if created_at and created_at < started_at: continue
    ...
    def _build_response_text_config(output_schema: dict[str, Any] | None, output_schema_name: str) -> dict[str, Any] | None:
        if not output_schema: return None
        return {
            "format": {
                "type": "json_schema",
    ...
    def _extract_function_calls(response: Response) -> list[dict[str, str]]:
        ...
        for item in getattr(response, "output", []) or []:
            if getattr(item, "type", None) != "function_call": continue
            call_id = str(getattr(item, "call_id", None) or getattr(item, "id", None) or "").strip()
            name = str(getattr(item, "name", None) or "").strip()
            arguments = getattr(item, "arguments", None)
            if call_id and name: calls.append({"call_id": call_id, "name": name, "arguments": arguments if isinstance(arguments, str) else json.dumps(arguments or {}, ensure_ascii=False, default=str)})
        return calls
    ...
    async def send_message_v2(self, *, input_text: str, conversation_id: str | None, bot_model: BotModel, known_input_tokens: int = 0, file_contents: list[tuple[str, bytes]] | None = None, image_contents: list[tuple[str, bytes]] | None = None, user_id: int | None = None, trace_id: str | None = None, function_tools: list[dict[str, Any]] | None = None, function_tool_executor: Callable[[str, dict[str, Any]], Awaitable[dict[str, Any]]] | None = None, max_tool_rounds: int = 4, output_schema: dict[str, Any] | None = None, output_schema_name: str = "ai_chat_output") -> dict[str, Any]:
        ...
        if not active_conversation_id: active_conversation_id = await self.create_conversation(user_id=user_id)
