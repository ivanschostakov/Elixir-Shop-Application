...
class OneCCatalogClient:
    ...
    def __init__(
        self,
        *,
        base_url: str | None = ONEC_ENTERPRISE_URL,
        username: str | None = ONEC_ENTERPRISE_LOGIN,
        password: str | None = ONEC_ENTERPRISE_PASSWORD,
        timeout_seconds: int = ONEC_REQUEST_TIMEOUT_SECONDS,
        stock_reserve: int = ONEC_STOCK_RESERVE,
    ) -> None:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if not self.is_configured():
            raise OneCIntegrationError("1C integration is not configured")
    ...
    async def fetch_odata(self, endpoint: str) -> list[dict[str, Any]]:
        ...
        for attempt in range(1, 4):
            try:
                ...
                self.log.info(
                    "Fetched 1C endpoint endpoint=%s status=%s records=%s attempt=%s seconds=%.2f",
                    endpoint,
    ...
    def _not_sold_in_tg(cls, extras: Any) -> bool:
        ...
        for item in extras:
            ...
            if property_key == cls.TG_NOT_SOLD_PROP_KEY and cls._is_truthy(item.get("Значение")):
                return True
        return False
    ...
    def _extra_text_by_keywords(extras: Any, keywords: list[str]) -> str | None:
        ...
        for item in extras:
            ...
            if text and any(keyword in text.casefold() for keyword in normalized_keywords):
                return text
        return None
    ...
    def _include_product(cls, record: dict[str, Any]) -> bool:
        ...
        if not ref_key:
            return False
        if not optional_str(record.get("КатегорияНоменклатуры_Key")):
            return False
        if cls._is_truthy(record.get("Недействителен")) or cls._is_truthy(record.get("DeletionMark")):
            return False
        if optional_str(record.get("ТипНоменклатуры")) != "Запас":
            return False
        if ref_key not in cls.FORCE_INCLUDE_PRODUCT_IDS and lower_optional_str(record.get("Parent_Key")) != cls.PARENT_KEY:
            return False
    ...
    def synthetic_variant_system_id(product_system_id: uuid.UUID) -> uuid.UUID:
        return uuid.uuid5(uuid.NAMESPACE_URL, f"elixir-shop:onec:synthetic-variant:{product_system_id}")

    @staticmethod
    def _fit_required(value: Any, max_length: int, *, fallback: str) -> str:
        return fit_text(value, max_length) or fallback[:max_length]
    ...
    async def fetch_products_1c(self) -> dict[str, dict[str, Any]]:
        ...
        for product in products:
            if not self._include_product(product):
                continue
            ...
            out[ref_key] = {
                ...
                "usage": self._extra_text_by_keywords(extras, USE_KEYWORDS),
                "expiration": self._extra_text_by_keywords(extras, EXPIRE_KEYWORDS),
            }
    ...
    async def fetch_prices_1c(self) -> dict[str, dict[str, Any]]:
        ...
        for price in prices:
            ...
            if not product_id or period is None:
                continue
            key = (product_id, feature_id)
            previous = latest.get(key)
            if previous is None or period > previous[0]:
                latest[key] = (period, price)

        result = {
            f"{product_id}_{feature_id}": {
                "product_onec_id": product_id,
    ...
    async def fetch_catalog_rows(self) -> tuple[list[OneCProductRow], list[OneCVariantRow], OneCCatalogSyncStats]:
        ...
        products, features, prices_map, balances_map = await asyncio.gather(
            self.fetch_products_1c(),
            self.fetch_features_1c(),
            self.fetch_prices_1c(),
            self.fetch_balances_1c(),
        )
        self.log.info(
            "1C catalog fetch completed products=%s features=%s prices=%s balances=%s",
            len(products),
            len(features),
            len(prices_map),
            len(balances_map),
        ...
        self.log.info(
            "1C catalog rows prepared products=%s variants=%s synthetic_variants=%s skipped_products_invalid_system_id=%s skipped_variants_invalid_system_id=%s",
            stats.fetched_products,
            stats.fetched_variants,
            stats.synthetic_variants,
            stats.skipped_products_invalid_system_id,
            stats.skipped_variants_invalid_system_id,
        )
        ...
        for product in products.values():
            ...
            rows.append(
                OneCProductRow(
                    ...
                    usage=fit_text(normalize_product_text(product.get("usage")), PRODUCT_USAGE_MAX_LENGTH),
                    expiration=fit_text(normalize_product_text(product.get("expiration")), PRODUCT_EXPIRATION_MAX_LENGTH),
                )
            )
        return rows
    ...
    def _merge_feature_rows(
        ...
        for product_id in products:
            if product_id in products_with_features:
                continue
            key = f"{product_id}_{self.EMPTY_FEATURE_KEY}"
            price = prices_map.get(key, {}).get("price", "0")
            balance = balances_map.get(key, {}).get("balance", "0")
            if (coerce_decimal(price) or Decimal("0")) <= 0 and (coerce_decimal(balance) or Decimal("0")) <= 0:
                continue
        ...
        for feature in features.values():
            ...
            rows.append(
                OneCVariantRow(
                    ...
                    sku=sku,
                    name=self._fit_required(feature.get("name"), VARIANT_NAME_MAX_LENGTH, fallback=name_fallback),
                    stock=self._available_stock(feature.get("balance")),
                    price=coerce_decimal(feature.get("price")) or Decimal("0"),
                )
            )
...
async def upsert_onec_catalog_rows(
    session: AsyncSession,
    products: list[OneCProductRow],
    variants: list[OneCVariantRow],
    stats: OneCCatalogSyncStats | None = None,
