...
> Статус: модуль подготовлен, но намеренно не установлен на production-сайт. Установка выполняется только после отдельного решения владельца.
...
3. Сгенерируйте отдельный секрет длиной не менее 32 байт:

   ```bash
   openssl rand -hex 32
   ```

4. Сохраните секрет в настройках модуля (команду выполнять из корня сайта, значение заменить):
...
5. Проверьте, что endpoint отвечает `405` на GET, а POST без корректной подписи - `401`:
