...
type CategoryForm = { name: string; description: string | null; archived: boolean; discount_percent: number }
...
export function CategoriesPage() {
  ...
  useEffect(() => {
    if (editing === "new") form.setFieldsValue({ name: "", description: null, archived: false, discount_percent: 0 })
    else if (editing) form.setFieldsValue({ name: editing.name, description: editing.description, archived: editing.archived, discount_percent: Number(editing.discount_percent) })
  }, [editing, form])
  ...
  const copy = locale === "ru" ? { title: "Категории", description: "Структура каталога, навигация и скидки витрины", add: "Добавить категорию", search: "Найти категорию", name: "Название", descriptionField: "Описание", discount: "Скидка на категорию, %", discountHint: "Применяется ко всем товарам категории, если она выгоднее скидки самого товара.", state: "Статус", updated: "Обновлено", active: "Активна", archived: "В архиве", edit: "Редактировать", save: "Сохранить" } : { title: "Categories", description: "Catalog structure, navigation, and storefront discounts", add: "Add category", search: "Find category", name: "Name", descriptionField: "Description", discount: "Category discount, %", discountHint: "Applies to all category products when it is better than the product discount.", state: "Status", updated: "Updated", active: "Active", archived: "Archived", edit: "Edit", save: "Save" }
