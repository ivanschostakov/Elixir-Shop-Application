...
export function ProductsPage() {
  ...
  const tableColumns = [
    ...
    { title: copy.state, key: "state", render: (_: unknown, row: Product) => <Space><Tag color={row.archived ? "default" : row.in_stock ? "green" : "orange"}>{row.archived ? "Archived" : row.in_stock ? copy.active : copy.out}</Tag></Space> },
  ...
  return (
    <div className="page-stack">
      ...
      <Card className="filter-card"><Space wrap><Input allowClear prefix={<SearchOutlined />} placeholder={copy.search} value={search} onChange={(event) => updateFilters({ q: event.target.value, page: 1 })} /><Space><Switch checked={archived} onChange={(value) => updateFilters({ archived: value ? "true" : undefined, page: 1 })} />{copy.archived}</Space></Space></Card>
      <TableToolbar
        resource="products"
        ...
        exportFilters={{ q: search, archived: archived || undefined }}
