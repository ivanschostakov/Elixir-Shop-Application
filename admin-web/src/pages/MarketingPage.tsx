...
export function MarketingPage() {
  ...
  const saveAutomation = useMutation({
    mutationFn: (values: Record<string, unknown>) => {
      if (!editingAutomation) throw new Error(copy.automationNotSelected)
  ...
  if (type === "campaign") return <InputNumber min={1} style={{ width: "100%" }} />
    if (selectedField === "customer_type") return <Select options={["first_time", "repeat", "no_orders"].map((value) => ({ value, label: domainLabel(value, locale) }))} />
    if (selectedField === "referral_status") return <Select options={["has_referral", "no_referral", "discount_active"].map((value) => ({ value, label: domainLabel(value, locale) }))} />
    if (selectedField === "platform") return <Select options={["ios", "android", "web"].map((value) => ({ value, label: domainLabel(value, locale) }))} />
    if (selectedField === "push_permission") return <Select options={["granted", "denied", "undetermined", "provisional", "unknown"].map((value) => ({ value, label: domainLabel(value, locale) }))} />
    if (selectedField === "lifecycle_stage") return <Select options={["new", "engaged", "interested", "high_intent", "customer", "repeat_customer"].map((value) => ({ value, label: domainLabel(value, locale) }))} />
    if (selectedField === "event_name") return <Select showSearch options={["app_opened", "product_viewed", "category_viewed", "search_submitted", "banner_clicked", "push_opened", "push_clicked", "cart_item_added", "cart_item_removed", "checkout_started", "checkout_failed", "order_created", "order_paid"].map((value) => ({ value, label: domainLabel(value, locale) }))} />
    return <Input />
  }
  ...
  return <div className="page-stack">
    ...
    {tab === "referrals" ? <>
      ...
      <Table<ReferralProfile> rowKey="id" loading={referrals.isLoading} dataSource={referralRows} pagination={{ pageSize: 25 }} columns={[
        ...
        {
          title: locale === "ru" ? "Единая программа" : "Unified program",
          key: "program",
          render: (_: unknown, row) => <Space wrap>
            <Tag color="blue">{locale === "ru" ? "Личная скидка активна" : "Personal discount active"}</Tag>
            <Tag color={row.partner_program_status === "active" ? "purple" : "default"}>
              {row.partner_program_status === "active"
                ? (locale === "ru" ? "Партнёр активен" : "Partner active")
                : (locale === "ru" ? "Партнёр до 100 000 ₽" : "Partner locked until RUB 100,000")}
            </Tag>
          </Space>,
        },
        {
          ...
          key: "bitrix",
          render: (_: unknown, row) => <div className="table-primary">
            <Tag color={row.bitrix_sync_status === "synced" ? "green" : row.bitrix_sync_status === "failed" ? "red" : "gold"}>
              {domainLabel(row.bitrix_sync_status, locale)}
            </Tag>
            <small>{row.bitrix_user_id ? `ID ${row.bitrix_user_id}` : "—"}</small>
          </div>,
        },
    ...
    <Drawer width={560} open={campaignDrawer} title={editingCampaign ? copy.edit : copy.newCampaign} onClose={() => setCampaignDrawer(false)} extra={<Button type="primary" loading={saveCampaign.isPending} onClick={() => void campaignForm.validateFields().then((values) => saveCampaign.mutate(values))}>{copy.save}</Button>}>
      <Form form={campaignForm} layout="vertical" requiredMark={false}>
        ...
        <Form.Item name="template_id" label={copy.template}><Select allowClear loading={campaignTemplates.isLoading} options={(campaignTemplates.data || []).map((template) => ({ value: template.id, label: `${locale === "ru" ? template.name_ru : template.name_en} · ${domainLabel(template.category, locale)}` }))} onChange={(value) => value ? applyTemplate(value) : null} /></Form.Item>
        ...
        {campaignPreview ? <Card className="campaign-preview-card" size="small">
          ...
          {campaignPreview.warnings.map((warning) => <Alert key={warning} type="warning" showIcon message={humanizeApiError(warning, 422, locale)} />)}
        </Card> : null}
      </Form>
    </Drawer>
