    ...
    <div className="page-stack">
      ...
      <Table<CrmLead>
        ...
        columns={[
          ...
          { title: copy.context, key: "context", render: (_: unknown, lead) => <Space direction="vertical" size={0}>{lead.conversation_id ? <Link onClick={(event) => event.stopPropagation()} to={`/communications?conversation_id=${lead.conversation_id}`}>{copy.conversation} #{lead.conversation_id}</Link> : null}{lead.converted_order_id ? <Link onClick={(event) => event.stopPropagation()} to={`/sales/orders/${lead.converted_order_id}`}>{lead.converted_order_code || `#${lead.converted_order_id}`}</Link> : null}{lead.product_id && lead.product_name && hasPermission("catalog.read") ? <Link onClick={(event) => event.stopPropagation()} to={`/catalog/products?product_id=${lead.product_id}`}>{lead.product_name}</Link> : lead.product_name || lead.category_name || "—"}</Space> },
