...
The AI Chat workspace also contains a consolidated usage report for 7, 30, 90 or 365 days. It keeps three sources separate: the application AI chat, the Bitrix website AI chat and Telegram AI bots. The report shows requests, users, successful and failed requests where the source persists failures, input/cached/output tokens, cache percentage, average tokens per response, response time, the current or recorded model, source-specific breakdowns, action funnels, trend charts and the twenty most active accounts in each source. Telegram cost is derived from its stored token ledger; application and Bitrix costs remain blank because those sources do not persist billing-grade historical price data. The cross-source user number is explicitly a sum, because the systems do not share a safe canonical identity for deduplication.
